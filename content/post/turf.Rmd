...
Hacía tiempo que no escuchaba hablar de él, pero ha vuelto a mi. TURF es un acrónimo anglosajón para Total Unduplicated Reach and Frequency. Este análisis es ampliamente utilizado en el mundo del marketing desde hace años, ya que nos da pistas y repsuestas a dos situaciones muy habituales en el análisis de cartera de productos, aunque su uso se puede extender a otros ámbitos muy diferentes:

- Por un lado, cuando una compañía dispone de una cartera de productos que quiere extender, pero teniendo las limitaciones de producción o prespuestarias que le impiden desarrollar sólo algunos de ellos, TURF les ayudará aleegir cuáles son los más apropiados.
...
El siguiente paso en el análisis es la tabla de duplicaciones. dado que el conjunto de repsuestas puede tratarse como una variable de tipo múltiple (binarizada) el cruce la variable por sí misma nos revelará todas las duplicaciones realizadas en la muestra entre todos los sabores.
...
Podemos observar en la tabla anterior que la mejor alternativa para tener mayor **alcance** a elegir es incoporar al portfolio es Strawberry, con un alcance de **`r round(turf$rchX[[1]]*100,2)`%**. Así hemos mejorado un alcance del **`r turf.1b`%**; ¿queremos sólo uno más o dos más?. Dando por buena esta combinación, ¿que aportaría más?
...
library(highcharter)
highchart() %>%
    hc_title(text = "Inrementos de alcance",style = list(fontSize = "2em")) %>% 
